...
description: 将经济学人网站文章转换为微信公众号格式并发布。支持：1) 浏览器抓取 body-text 标签内容 2) 根据原文 initial 标签判断首字母大写 3) 自动检测缩写词并添加 small 标签 4) 上传图片到微信素材库 5) 创建草稿。Use when user wants to: (1) Convert Economist articles to WeChat Official Account format using body-text tags, (2) Apply initial capital format based on original initial tags, (3) Auto-detect abbreviations and wrap with small tags, (4) Upload images and create drafts.
---
...
```javascript
// 在浏览器控制台中运行，提取所有 body-text 段落
const bodyTexts = document.querySelectorAll('body-text.svelte-qapeiq');
return Array.from(bodyTexts).map((bt, i) => {
  const initial = bt.querySelector('span[data-caps="initial"]');
  const text = bt.textContent.trim();
  if (text.length < 20 || text === 'placeholder') return null;
  return {
    index: i,
    has_initial: !!initial,
    initial_letter: initial ? initial.textContent : null,
    text: text  // 纯文本，不含 HTML 标签
  };
}).filter(x => x !== null);
```

**关键规则：**
- ✅ **分段**：使用 `<body-text class="svelte-qapeiq">` 标签
- ✅ **首字母**：只有原文有 `<span data-caps="initial">` 标签的段落才使用首字母大写格式
- ✅ **缩写词**：从纯文本自动检测缩写词，用自己的 small 标签格式包裹（不使用原文的 small 标签）

#### 步骤 2：获取封面图片
经济学人图片有防盗链，需要通过浏览器下载：
